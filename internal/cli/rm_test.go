...
func TestRunRmReportsDirtyWorktree(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not found in PATH")
	}
	...
	if got := out.String(); got != "wt rm: worktree has untracked files (e.g. scratch.txt). Use -f to remove or clean.\n" {
...
func TestRunRmForceRemovesDirtyWorktree(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not found in PATH")
	}
	...
	code := Run([]string{"rm", "-f", "feature/one"}, &out, &errOut)
